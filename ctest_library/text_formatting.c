 ...
 * that, the prefix of the first line can be specified separately. 
 ...
 *        (size_t) text_length --> The length of the text that will be read 
 ...
 *        (size_t) line_length --> The length of each formatted line.
 *
	...
	while(state != 2)
	{
		switch(state)
		{
			case 0:
				...
				break;
			case 1:
				if(!is_white_space(text[i]) && text[i] != '\0') i++;
				else if (text[i] == '\0') 
				{
					end = i;
					num_of_words++;
